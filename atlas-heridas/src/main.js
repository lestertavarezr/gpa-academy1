...
function options(step) {
  ...
  if(step.kind==='dressing') return '<div class="panel-minihead">ARMARIO DE APÓSITOS <span>TOCA O ARRASTRA AL LECHO</span></div><div class="dressing-options">'+orderedOptions(step).map(id=>{const d=dressingFor(id), cls=optionState(step,id); return '<button class="dressing-option '+cls+'" draggable="'+!state.feedback+'" data-action="pick" data-id="'+id+'" aria-pressed="'+state.picks.includes(id)+'" '+lock+'><img src="'+d.image+'" alt="Fotografía de '+E(d.name)+'" width="64" height="62" loading="lazy"/><span><strong>'+E(d.name)+'</strong><small>'+E(d.detail)+'</small></span><i aria-hidden="true">'+(cls.includes('is-wrong')?'✕':cls.includes('is-correct')||state.picks.includes(id)?'✓':'+')+'</i></button>';}).join('')+'</div>';
